<template>
  <div @click="waitTime=0">
    ...
    <div class="fullpage2" :class="{indexOpen:rightOpen}">
      ...
      <div   @mousemove="MouseMove" class="imageContent" @click="goOther">
        <div v-show="whiteShow" class="whitePage" :class="{whiteOut:whiteOut}"></div>
        <img :src="activeSrc" alt="">
      </div>
      <p class="imageIndex imageIndexBottom"  @mouseenter="MouseLeave">
        <span @click="openRightIndex">Index</span>
        <span class="midLine1"></span>
        <span @click="OpenIndex">Thumbs</span>
      </p>
      <p @click="goOther" class="mouseIndex"><span>{{groupName}} {{imgIndex+1}}/{{groupTotal}}</span></p>
    </div>
    <div class="rightIndexBg" :class="{rightIndexBgActive:rightOpen}" @click="closeRightIndex"></div>
    <div class="rightIndex " @mouseenter="rightIndexHover=true" @mouseout="rightIndexHover=false" :class="{rightIndexActive:rightOpen,rightIndexHover:rightIndexHover}">
      ...
      <div class="groupDiv">
        <ul>
          <li  @mouseenter="groupEnter(index)" @mouseleave="groupLeave(index)" v-for="item,index in imgConfig" @click="gotoIndex(item,index)" :class="{bold:index==groupIndex}">
    ...
    methods: {
      ...
      goNext() {
        ...
        if (this.imgIndex < this.imgConfig[this.groupIndex].imgList.length - 1) {
          ...
          document.querySelector('.mouseIndex').style.top='50%';
          document.querySelector('.mouseIndex').style.left='50%';
          document.querySelector('.mouseIndex').style.fontSize='24px';
          setTimeout(function () {
          ...
          setTimeout(function () {
            ...
            document.querySelector('.mouseIndex').style.fontSize='14px';
          }, 2000)
        }
      },
      goBefore() {
        ...
        if (this.imgIndex > 0) {
          ...
          document.querySelector('.mouseIndex').style.left='50%';
          document.querySelector('.mouseIndex').style.fontSize='24px';
          document.querySelector('.mouseIndex').style.transition='1s';
          setTimeout(function () {
          ...
          setTimeout(function () {
            that.imageIndexB = true;
            that.mouseActive = true;
            document.querySelector('.mouseIndex').style.transition='0.1s';
            document.querySelector('.mouseIndex').style.fontSize='14px';
          }, 2000)
        }
      },
      gotoIndex(item, index) {
        ...
        that.imageIndexB = false;
        that.mouseActive = false;
        document.querySelector('.mouseIndex').style.opacity='1';
        document.querySelector('.mouseIndex').style.top='50%';
        document.querySelector('.mouseIndex').style.left='50%';
        document.querySelector('.mouseIndex').style.fontSize='24px';
        document.querySelector('.mouseIndex').style.transition='1s';
      ...
      changeMouse(){
        document.querySelector('.mouseIndex').style.transition='0.5s';
        document.querySelector('.mouseIndex').style.opacity=1;
      },
      MouseLeave(){
        document.querySelector('.mouseIndex').style.transition='0.5s';
        document.querySelector('.mouseIndex').style.opacity=0;
      },
      MouseMove(e){
        if(this.mouseActive){
          document.querySelector('.mouseIndex').style.transition='0.1s';
          document.querySelector('.mouseIndex').style.opacity=1;
          document.querySelector('.mouseIndex').style.top=e.pageY-30+'px';
          document.querySelector('.mouseIndex').style.left=e.pageX-30+'px';
        }
      },
      groupEnter(index){
        this.hoverSrc=this.imgConfig[index].imgList[0];
      },
      groupLeave(){
        this.hoverSrc=''
      }
